...
select product_name from "assignment".products where  price > 500
...
select product_id, product_name, category  from "assignment".products where category  = 'Electronics'

-- 9. Write a query to find the highest price from the `Products` table.
select max(price) as PRICE from "assignment".products

-- 10. Write a query to count the number of sales for each product from the `Sales` table.
select p.product_name, s.product_id, sum(quantity_sold) as total_sales from "assignment".sales s 
...
select min(p.price) from "assignment".products p 

-- 13. Write a query to find customers who have purchased products with a price greater than 1000.
select c.first_name, c.last_name, s.customer_id, p.product_name from "assignment".sales s 
...
select p.product_name ,sum(s.total_amount) as sales_amount from "assignment".sales s 
join "assignment".products p on p.product_id = s.product_id
group by p.product_name


-- 15. Write a query to join the `Customers` and `Sales` tables and find the total amount spent by each customer.
select c.first_name, c.last_name, sum(s.total_amount) as spent from "assignment".customers c 
...
select c.first_name, c.last_name, p.product_name, sum(s.quantity_sold) as quantityBought from "assignment".customers c 
join "assignment".sales s on s.customer_id = c.customer_id 
join "assignment".products p on p.product_id = s.product_id 
...
select c1.customer_id, c1.first_name, c1.last_name, c2.customer_id, c2.first_name, c2.last_name from "assignment".customers c1
...
select p.product_name, count(s.product_id) as NumberofSales from "assignment".sales s
join "assignment".products p on p.product_id = s.product_id
group by p.product_name
